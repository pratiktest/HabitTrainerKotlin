...
import kotlinx.android.synthetic.main.content_create_habit.*
...
class CreateHabitActivity : AppCompatActivity() {
    ...
    fun storeHabit(v:View){
        if(et_title.isBlank() || et_description.isBlank()){
            Log.d(TAG, "title of description is blank")
            displayErrorMessage("Add title and description to habit")
        }else if(imageBitmap == null){
            Log.d(TAG, "no habit stored")
            displayErrorMessage("Add picture to habit")
        }

        val title = et_title.text.toString()
        val description = et_description.text.toString()

        //we check for nullability above for imageBitmap
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode == CHOOSE_IMAGE_REQUEST && resultCode == Activity.RESULT_OK && data != null && data.data != null){
            ...
            Log.d(TAG, "image was choosen by user")
            val  bitmap = tryReadBitmap(data.data)

            bitmap?.let{
                iv_image.setImageBitmap(bitmap)
                this.imageBitmap = bitmap
                Log.d(TAG, "Read bitmap image and updated view")
            }
